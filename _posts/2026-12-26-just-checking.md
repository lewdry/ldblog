...
These are predictable vulnerabilities where design affordances and human psychology interact to produce contexts where conventional verification strategies fail.
...
The research points to some practical directions, but honestly, they're easier to identify than to implement.

For one, we need better ways to show people where information is coming from and how confident the AI actually is about its answer. But as the research showed, signalling uncertainty makes people trust the system less—even when it's being more honest. So how do you design interfaces that communicate "I'm not totally sure about this" without undermining the whole interaction?

There's also the platform-level stuff: better metadata about where information originated, ranking systems that take provenance into account. But Kuznetsova et al. (2025) showed that AI performance varies massively by language and topic. Any governance system needs to account for that, or we're just building infrastructure that works well for English-speakers asking about Western political contexts while everyone else gets left with worse information.

Then there's the question of whether we can actually teach people to use these systems better. Training programs, explainers, literacy interventions—all of that needs testing across different groups, not just the educated, tech-comfortable populations that show up in most studies.

Methodologically, we're also still figuring out how to study this properly. Most of what I've reviewed here is either experimental (artificial lab settings) or survey-based (what people say they do, not what they actually do). We need longitudinal studies that follow people over time, cross-cultural work that doesn't assume everyone behaves like a standard population, and studies that actually measure verification behavior in the wild.
