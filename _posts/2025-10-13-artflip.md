...
#### 4. Data Sources & APIs
- The museums’ APIs are amazing, but occasionally confusing and all have their own quirks.
- The first version of Artflip (called **RandyMet**) just hotlinked random images from the Met. It would run an API call through a proxy, ask for a huge set of artworks, a big clumsy request, then select one of the results at random to display. It took ages. You'd face upto 60 seconds of a spinning progress wheel before seeing an artwork. Often the request would just time out.
- This method worked, badly, until it didn't. As a result of my crude API calls, my IP address got throttled, and then blocked by the Met (sorry Met!)... which required a re-write.
...
I tried using ChatGPT to write some scripts but found it lazy and unreliable. It would regularly 'forget' parts of my code, or suggest a generic solution to a problem I wasn't having. When questioned, it would respond that it hadn't actually read the code I'd shared, but guessed instead. I'm not sure if that's a technical limitation, or an economic belt-tightening by OpenAI.

I tried using Google's Gemini but found it similarly unhelpful. In one case it just told me it couldn't help. Which I suppose is preferable to confident hallucination.

I relied on GitHub Copilot to **write my commit messages**, (for non-nerds reading this, a commit message is a technical note-to-self). In fact, I completely outsourced this. I think this is the most exciting AI use case: automating drudgery, summarising a range of changes across a codebase into a human-readable short message.

I wouldn't say I'm a huge AI fan (for the record, no AI was used to write this blog), but I'm happy to acknowledge I wouldn't have been able to put Artflip together without it. So, thanks Claude.
...
- Automation can make a product feel alive without constant attention. Artflip's latest Friday-night-update took place while I was splashing around at a beach with my kids, enjoying a lighter spring evening.
...
- Tidying up my scripts & front-end to make a re-usable and open source tool for other oddball collectors.
